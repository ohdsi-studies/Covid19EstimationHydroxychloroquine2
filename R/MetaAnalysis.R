...
doMetaAnalysis <- function(studyFolder,
  ...
  # drop bad TAR in OptumEHR and outcomes in databases with no IP capture
  drops <- 
    (allResults$databaseId == "OptumEHR" & allResults$analysisId == 1) | # panther on-treatment
    (allResults$databaseId %in% c("AmbEMR", "CPRD", "DAGermany", "IMRD") & allResults$outcomeId %in% c(16526, 15636)) # databases with no IP
  allResults <- allResults[!drops, ]
  
  # blind estimates that don't pass diagnostics 
  blinds <- 
    (allResults$databaseId == "CPRD" & allResults$targetId == 137) | 
...
getAllControls <- function(outputFolder) {
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "Covid19EstimationHydroxychloroquine2", mustWork = TRUE)
